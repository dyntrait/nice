...
use nice_core::consts::NICE_PREFIX;
use nice_core::{
    UUID4, UnixNanos,
...
fn call_python_with_time_event(event: TimeEvent, callback: &Py<PyAny>) {
    use nice_core::python::IntoPyObjectniceExt;
    ...
    Python::attach(|py| {
        ...
        match callback.call1(py, (capsule,)) {
            Ok(_) => {}
            Err(e) => eprintln!("{nice_PREFIX} Error on callback: {e:?}"),
        }
    });
}
...
mod tests {
    ...
    use nice_core::{UnixNanos, time::get_atomic_clock_realtime};
    use rstest::*;
    use ustr::Ustr;
