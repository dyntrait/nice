...
//
//  @File         : runtime.rs.rs
//  @Author       : dyntrait Created On 2026/1/5 15:57
//  @Description  : 
//
...
//! The centralized Tokio runtime for a running NICE system.
//!
//! # Design Rationale
//!
//! NICETrader uses a single global Tokio runtime because:
...
//! - Worker thread count is configurable via the `NICE_WORKER_THREADS` environment variable.
//!
//! # Python Support
//!
...
const NICE_WORKER_THREADS: &str = "NICE_WORKER_THREADS";
...
///
/// The number of OS threads is configured using the `NICE_WORKER_THREADS`
...
fn initialize_runtime() -> tokio::runtime::Runtime {
    ...
    let worker_threads = std::env::var(NICE_WORKER_THREADS)
        .ok()
...
/// Returns a reference to the global NICE Tokio runtime.
///
